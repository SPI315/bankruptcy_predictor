...
async def registration(user_data: UserRegister, session=Depends(get_db)):
    ...
    if user_exists:
        ...
        user = User()
        user.user_add(session, user_data)
...
async def check_balance(
    id: int, session=Depends(get_db), user: str = Depends(authenticate)
):
    user = User()
    return user.check_balance(session, user_id=id)


@user_router.delete("/delete/{id}")
async def delete_user(id: int, session=Depends(get_db)):
    user = User()
    user.user_del(session, user_id=id)
    return "Пользователь удален"


@user_router.get("/get_by_email")
async def get_by_email(
    email: str, session=Depends(get_db), user: str = Depends(authenticate)
):
